...
(function () {
  ...
  // Only one promotional badge is ever shown per card. When a product has
  // more than one, the highest-priority badge wins.
  const BADGE_PRIORITY = ["Best Seller", "Editor's Choice", "Trending", "Dermatologist Recommended", "Budget Pick"];

  function primaryBadge(badges) {
    if (!badges || !badges.length) return null;
    return BADGE_PRIORITY.find((candidate) => badges.includes(candidate)) || null;
  }
  ...
  function initHomeSections() {
    ...
    const byBadge = (badge) => products.filter((p) => (p.badges || []).includes(badge));
    const recent = [...products].sort((a, b) => b.id - a.id);

    mountGrid(qs("#featured-grid"), byBadge("Editor's Choice").concat(byBadge("Trending")).slice(0, 4));
    mountGrid(qs("#trending-grid"), byBadge("Trending").slice(0, 4));
    mountGrid(qs("#bestsellers-grid"), byBadge("Best Seller").slice(0, 8));
    mountGrid(qs("#recent-grid"), recent.slice(0, 4));
  }
  ...
  function initProductPage() {
    ...
    qs("#product-badges").innerHTML = badgesMarkup(product.badges);
    ...
    qs("#product-match-ring").innerHTML = matchRingMarkup(product.matchScore);
    qs("#product-overview").textContent = product.overview;
    qs("#product-skin-type").textContent = product.skinType.join(", ");
    ...
    qs("#product-pros").innerHTML = product.pros.map((p) => `<li><i class="fa-solid fa-circle-check" aria-hidden="true"></i>${p}</li>`).join("");
    qs("#product-cons").innerHTML = product.cons.map((c) => `<li><i class="fa-solid fa-circle-minus" aria-hidden="true"></i>${c}</li>`).join("");
    qs("#product-review-text").textContent = product.review;
    qs("#product-ingredients").innerHTML = product.ingredients.map((i) => `<li>${i}</li>`).join("");
    qs("#product-how-to-use").textContent = product.howToUse;
    qs("#product-who-for").textContent = product.whoFor;
    qs("#product-who-avoid").textContent = product.whoAvoid;
